...
| **72** | Text as vector curves (convert-to-outlines) | **New.** Text we can't embed as live type but must keep crisp draws as **filled/stroked glyph curves** (the font's own outlines, quad→cubic), not raster. Live for **stroked lettering** end-to-end (gs-verified: no font, no image — pure vector). The core handles any run + letter-spacing + transforms; rotated/arc/bubble wiring is the next step. Not selectable (curves carry no text — by design). |
...
| **65** | Spot / Pantone plates | **Now real end-to-end.** Create a named spot swatch ("+ Spot"), fill a solid rectangle with it, set the spot policy to "Preserve named spots" → the export emits a real `/Separation` plate and knocks that fill out of the process raster. **gs-tiffsep-verified** (a "PANTONE 185 C" plate appears) and still valid PDF/X-4; preflight discloses exactly which spots are kept vs converted. Caps: solid, upright, un-stroked, full-opacity rectangles only (a spot on a stroke / rounded / tinted shape / text still converts, disclosed); not yet proven in a real RIP/press by you. |
...
| 2 | **Spot color — broaden coverage** | Medium | Solid spot fills now ship end-to-end. Still convert (disclosed): spot on strokes, rounded/rotated/tinted shapes, and spot TEXT (needs raw `/Separation` text ops). Also: prove one plate in a real RIP. |
...
| 4 | **Vector-text reach** | Medium | Single-column is now the default and **stroked lettering now outlines to vector curves** instead of rasterizing. Still to reach vector: rotated, arc/on-a-curve, and speech-bubble text (the outline core handles these — the exporter wiring + editor-geometry match is the remaining work, and needs a browser render to verify pixel alignment). |
