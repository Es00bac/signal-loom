...
export function imageTextCanvasFont(style: Pick<ImageTextLayerStyle, 'fontFamily' | 'fontSize' | 'fontWeight' | 'fontStyle' | 'fontVariantCaps'>): string {
  const caps = style.fontVariantCaps && style.fontVariantCaps !== 'normal' ? `${style.fontVariantCaps} ` : '';
  return `${style.fontStyle} ${caps}${style.fontWeight} ${style.fontSize}px ${style.fontFamily}`;
}
